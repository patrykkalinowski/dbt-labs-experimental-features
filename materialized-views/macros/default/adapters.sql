{% macro create_materialized_view_as(relation, sql, config) %}
    {{ return(adapter.dispatch('create_materialized_view_as', 'dbt_labs_materialized_views')(relation, sql, config)) }}
{% endmacro %}
...
{% macro refresh_materialized_view(relation, config) %}
    {{ return(adapter.dispatch('refresh_materialized_view', 'dbt_labs_materialized_views')(relation, config)) }}
{% endmacro %}
